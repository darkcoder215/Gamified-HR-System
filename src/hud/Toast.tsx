...
export default function Toast() {
  const [msg, setMsg] = useState<{ text: string; level?: number } | null>(null);

  useEffect(() => {
    const onLocked = (p: { stationId: string }) => {
      const z = zoneForStation(p.stationId as never);
      sfx('wrong');
      setMsg({ text: z ? `«${z.nameAr}» مغلق — يفتح عند المستوى` : 'هذا المكان مغلق حاليًا', level: z?.unlockLevel });
      window.setTimeout(() => setMsg(null), 2600);
    };
    EventBus.on('station:locked', onLocked);
    return () => EventBus.off('station:locked', onLocked);
  }, []);
      ...
      {msg && (
        ...
        >
          <div className="flex items-center gap-2 rounded-pill bg-black/85 px-4 py-2 font-ui text-sm font-bold text-white shadow-float backdrop-blur">
            ...
            {msg.text}
            {msg.level !== undefined && <span className="num"><NumberText value={msg.level} /></span>}
          </div>
        </motion.div>
      )}
    </AnimatePresence>

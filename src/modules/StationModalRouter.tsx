import { useEffect } from 'react';
import { Swords, ClipboardList, TrendingUp, Trophy, Building2 } from 'lucide-react';
import type { StationId } from '@/types';
import { useGameStore } from '@/state/store';
import { EventBus } from '@/game/EventBus';
import Modal from '@/ui/Modal';
import AssessmentArena from './arena/AssessmentArena';
import QuestBoard from './quests/QuestBoard';
import CareerLadder from './promotion/CareerLadder';
import LeaderboardHall from './leaderboard/LeaderboardHall';
import OrgHierarchy from './org/OrgHierarchy';
import InteriorScene from './interior/InteriorScene';
import { isStationUnlocked } from '@/data/zones';

interface StationMeta {
  title: string;
  subtitle: string;
  accent: string;
  icon: JSX.Element;
  Component: () => JSX.Element;
  maxWidth: string;
}

const META: Record<StationId, StationMeta> = {
  arena: {
    title: 'Assessment Arena',
    subtitle: 'Test your skills and earn XP',
    accent: 'var(--color-green)',
    icon: <Swords size={24} />,
    Component: AssessmentArena,
    maxWidth: '680px',
  },
  quests: {
    title: 'Quest Board',
    subtitle: 'Complete development quests and level up your skills',
    accent: 'var(--color-blue)',
    icon: <ClipboardList size={24} />,
    Component: QuestBoard,
    maxWidth: '760px',
  },
  career: {
    title: 'Promotion Tower',
    subtitle: 'Your career path and promotion opportunities',
    accent: 'var(--color-amber)',
    icon: <TrendingUp size={24} />,
    Component: CareerLadder,
    maxWidth: '680px',
  },
  leaderboard: {
    title: 'Leaderboard Hall',
    subtitle: 'Your ranking among colleagues and your badges',
    accent: 'var(--color-burgundy)',
    icon: <Trophy size={24} />,
    Component: LeaderboardHall,
    maxWidth: '680px',
  },
  org: {
    title: 'Org Chart',
    subtitle: 'The POWR team and your place in it',
    accent: 'var(--color-charcoal)',
    icon: <Building2 size={24} />,
    Component: OrgHierarchy,
    maxWidth: '720px',
  },
};

export default function StationModalRouter() {
  const activeStation = useGameStore((s) => s.activeStation);
  const openStation = useGameStore((s) => s.openStation);
  const closeStation = useGameStore((s) => s.closeStation);

  // Phaser → React: a station was entered (ignore still-locked districts).
  useEffect(() => {
    const handler = (p: { stationId: StationId }) => {
      if (!isStationUnlocked(p.stationId, useGameStore.getState().player.level)) return;
      openStation(p.stationId);
    };
    EventBus.on('station:enter', handler);
    return () => {
      EventBus.off('station:enter', handler);
    };
  }, [openStation]);

  const meta = activeStation ? META[activeStation] : null;

  return (
    <Modal
      open={!!meta}
      onClose={closeStation}
      title={meta?.title}
      subtitle={meta?.subtitle}
      icon={meta?.icon}
      accent={meta?.accent}
      maxWidth={meta?.maxWidth}
    >
      {meta && activeStation ? (
        <>
          <InteriorScene id={activeStation} title={meta.title} />
          <meta.Component />
        </>
      ) : null}
    </Modal>
  );
}
